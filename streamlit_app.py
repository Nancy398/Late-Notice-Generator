...
def format_amount_in_words(amount):
    ...
    dollars = int(amount)
    cents = int(round((amount - dollars) * 100))
    dollar_words = p.number_to_words(dollars).replace(",", "").title()
    return f"{dollar_words} Dollars and {cents}/100 Cents" if cents else f"{dollar_words} Dollars"
...
def fill_pdf(data):
    ...
    for page in pdf:
        for key, value in data.items():
            matches = page.search_for(f"{{{{{key}}}}}")
            for match in matches:
                page.draw_rect(match, color=(1, 1, 1), fill=(1, 1, 1))
                page.insert_text((match.x0, match.y1 - 2.5), value, fontsize=12, fontname="Times-Roman", color=(0, 0, 0))
    buffer = BytesIO()
    pdf.save(buffer)
    buffer.seek(0)
    return buffer
...
def merge_pdfs(generated_pdf, uploaded_pdf):
    writer = PdfWriter()
    for pdf_file in [generated_pdf, uploaded_pdf]:
...
if st.button("生成 PDF"):
    if not uploaded_file:
        st.warning("Please upload a tenant ledger PDF.")
    else:
        date_str = datetime.now().strftime("%B %d, %Y")
        amount_words = format_amount_in_words(amount)
        formatted_amount = f"{amount:,.2f}"
        data = {
            "Full Name": full_name,
            "Address": address,
            "Postal": postal,
            "Amount Words": f"{date_str} is {amount_words} (${formatted_amount}).",
            "Date": date_str,
            "Full Address": f"{address}, Los Angeles, CA, {postal}",
            "gender": f"{title} {full_name},"
        }
        filled_pdf = fill_pdf(data)
        merged_pdf = merge_pdfs(filled_pdf, uploaded_file)
        st.download_button("Download Merged PDF", merged_pdf, f"Late Notice - {full_name}.pdf", "application/pdf")
